...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    cout << argv[0] << " [benchmark input file]" << endl;
    return 0;
  }

  const string benchmark_input = argv[1];
  const string benchmark_output = "output_" + benchmark_input;

  cout << "input: " << benchmark_input << endl;
  cout << "milliseconds:" << endl;
  for (int times = 0; times < 10; times++) {
    ifstream fin(benchmark_input);
    ofstream fout(benchmark_output);
